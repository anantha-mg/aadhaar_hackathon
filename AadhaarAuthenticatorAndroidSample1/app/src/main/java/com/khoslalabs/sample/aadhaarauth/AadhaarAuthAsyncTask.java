...
public class AadhaarAuthAsyncTask extends AsyncTask<String, Void, AuthResponse> {
	...
	protected void onPostExecute(AuthResponse response) {
		...
		alertDialog.setTitle("Note to Invigilator:");

		String msg = "Auth Status: " + (response.isSuccess() ? "success" : "fail")
		        + (null != response.getReferenceCode() ? "\nRefcode: " + response.getReferenceCode() : "");

		msg = "Aadhaar result : " + response.getAadhaarResult() +
				" Status code :  " + response.getStatusCode() +
				" Error code : " + response.getErrorCode() +
				" Error Type : " + response.getErrorType() +
				" Status : " + response.isSuccess()	;

		String successMessage = "Biometric verification successful. Allow student to write exam";
		String failureMessage = "Biometric verification failed. Clean if device has dust and try again. " +
				"If it repeats, verify student details and take action [expel from exam]";
